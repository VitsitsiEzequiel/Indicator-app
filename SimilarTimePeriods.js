...
import {
    MDBRow, MDBCol, MDBCard, MDBCardBody, MDBDropdown, MDBDropdownToggle, MDBBox,
    MDBBtn, MDBCardHeader, MDBDropdownMenu, MDBTable, MDBTableBody, MDBTableHead,
    MDBDropdownItem
} from 'mdbreact';
...
const SimilarTimePeriods = (props) => {
    ...
    const handleIndicatorClick = (e) => {
        ...
        const filterIndicator = indicators.filter(indicator => indicator.id === e.target.value && indicator)[0]
    ...
    useEffect(() => {
        // setLoading(true)
        getInstance().then(d2 => {
            const endPoint = "/organisationUnits.json?level=1&fields=id,name,displayName,children[id,name,displayName,children[id,name,displayName,children[id,name,displayName,children[id,name,displayName,children[id,name,displayName,children[id,name,displayName,children]]]]]],level&paging=false";
    ...
    const getChildren = children => {
        return children && children.children.map(child => {
            return <TreeItem
                key={child.id}
                name={child.name}
                nodeId={child.id}
    ...
    const RenderChildren = props => {
        return props.orgUnits !== undefined
    ...
    const handleCompare = () => {
        ...
        if (thisPeriod === "select a period this year") {
            ...
            makeComparison(thisPeriod, lastPeriod, selectedIndicator, period, units.join(";"));
        }
    ...
    const makeComparison = (thisPeriod, lastPeriod, indicator, timePeriod, orgUnit) => {
        ...
        const returnback = (result) => {
            ...
            if (result == null) {
                ...
                var periods = [result.metaData.items[period1].name, result.metaData.items[period2].name];
                setYears(periods);
                ...
                if (result.rows == null || result.rows.length === 0) {
                    alert("No data available, try again with a different orgUnit, period or indicator!");
                    // rows.indicatorValue = "N/A";
                    // value.push({"value" : "N/A"})
                }
        ...
        <MDBBox display="flex" justifyContent="center" >
            <MDBCol className="mb-5" md="10" style={{ width: "350px" }}>
                <MDBCard >
                    ...
                    <MDBCardBody  >
                        <MDBTable id="tableDiv" hover bordered responsive className="border-light border" hover small>
                            <MDBTableHead>
                                <tr>
                                    <th>
                                        {/* Period */}
                ...
                <Grid container justify="center" alignItems="center" style={{ margin: 10 }}>
                    ...
                    <Button onClick={() => { downloadPDF(selectedIndicator.displayName) }} type="primary">
                        Download {showDownloading ? <div className="spinner-border mx-2 text-white spinner-border-sm" role="status">
    ...
    return (
        <div className="period">
            ...
            <div style={{ maxHeight: "100px", marginLeft: "5px", marginRight: "5px", paddingTop: "5px" }}>
                <Button type="primary" onClick={handleBtnClick}> Home </Button>
            </div>
            <hr />
                ...
                <div className="jumbotron">
                    ...
                    <form action="#" onSubmit={handleSubmit} className="MyForm">
                        <MDBRow style={{ marginLeft: "2rem", height: 400 }}>
                                    ...
                                    <MDBCardBody>
                                        <Scrollbars style={{ width: 350, height: 350 }}>
                                            <div>
                                                ...
                                                <label className="grey-text ml-2"><strong>Select Organisation units</strong> </label>
                                                <TreeView
                            ...
                            <MDBCol md="7" style={{ display: "flex", paddingLeft: "6rem", height: 400 }}>
                                <MDBCard>
                                    <MDBCardBody>
                                        <hr style={{ width: 450 }} />
                                        <label className="control-label">Indicator :</label>&nbsp;&nbsp;&nbsp;

                                        <select name="mark_location" id="mark_location"
                                            style={{ width: '300px' }} onChange={handleIndicatorClick}>

                                            <option>Select Indicator</option>

                                            {
                                                indicators && indicators.map(indicator => {
                                                    return <option value={indicator.id} >{indicator.displayName}</option>
                                                })
                                            }
                        ...
                        <MDBRow style={{ marginLeft: "3rem" }}>
                            <MDBCol size="5">
                                <MDBRow >
                                    <MDBRow >
                                        <MDBDropdown className=" myDropDown" style={{ width: "400px" }} size="sm">
